...
void main(void)
{
    ...
    IRQ_clearAll(); // This function clears all the interrupts.
    ...
    set_i2s2_slave();		// Set I2S
    AIC3204_init(SF48KHz, DAC_GAIN, (Uint16)ADC_GAIN);  // Set AIC3204
	enable_i2s2();

    enable_dma_int();		// Set up and enable DMA
    set_dma1_ch0_i2s2_Lout(XMIT_BUFF_SIZE);
    set_dma1_ch1_i2s2_Rout(XMIT_BUFF_SIZE);
    set_dma1_ch2_i2s2_Lin(XMIT_BUFF_SIZE);
    set_dma1_ch3_i2s2_Rin(XMIT_BUFF_SIZE);
